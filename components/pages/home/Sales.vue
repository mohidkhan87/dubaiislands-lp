<template>
  <div class="lg:bg-[#F7F7F4] -mt-1">
    <div
      class="bg-[#1EBEF1] text-white text-2xl lg:text-4xl 2xl:text-6xl text-center font-semibold px-2 py-8 relative"
    >
      <BaseContainer>
      <h3 class="text-xl md:text-3xl">Invest in RERA-Certified Project with a developer renowned for delivering on-time</h3>
      </BaseContainer>
    </div>
    <BaseContainer>
      <div class="grid grid-cols-2 gap-y-4 xs:gap-6 pt-12 lg:pt-8 pb-8">
        <div class="text-center">
          <h3 class="text-lg xs:text-2xl sm:text-3xl lg:text-4xl 2xl:text-6xl font-semibold">1-4</h3>
          <p class="text-sm xs:text-base 2xl:text-xl font-medium">Bedroom Apartments</p>
        </div>
        <div class="text-center">
          <h3 class="text-lg xs:text-2xl sm:text-3xl lg:text-4xl 2xl:text-6xl font-semibold">Exclusive</h3>
          <p class="text-sm xs:text-base 2xl:text-xl font-medium">Payment Plans</p>
        </div>
      </div>
      <div class="block md:hidden mb-8 border-2 border-[#1EBEF1] rounded-lg">
        <Form />
      </div>
    </BaseContainer>
  </div>
</template>
